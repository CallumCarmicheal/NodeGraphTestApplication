...
namespace WpfNodeGraphTest.NGraph.Models.Functions {
	[Node]
	[CNodeDescriptor("Function: Print String")]
	[NodeFlowPort("Input", "", true)]
	[NodeFlowPort("Output", "", false)]
	public class FnPrintString : CNodeBase {
		// ====================================================
		// ====  Properties

		[NodePropertyPort(displayName: "Text", isInput: true, valueType: typeof(string), defaultValue: "", hasEditor: true)]
		public string Text;
		...
		public override string CompileAsJavascript() {
			...
			if (InputPropertyPorts[0].Connectors.Count > 0 && InputPropertyPorts[0].Connectors[0].StartPort != null) {
				...
				output = string.Format(format, $"\"{Text}\"");
			}
